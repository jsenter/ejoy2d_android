...
#elif __ANDROID__
static const char* TTFONT = "/sdcard/ejoy2d/FZCY_GBK.ttf";
#else
...
static void __fault(int errcode, const char * msg, const char *file, int line) {
    if (errcode) {
        __android_log_print(ANDROID_LOG_ERROR, "EJOY2D", "err(%d): %s, error occured in file %s, line %d\n\n\t have a look at fterrdef.h\n",errcode, msg, file, line);
    } else {
        __android_log_print(ANDROID_LOG_ERROR, "EJOY2D", "err: %s, occured in file %s, line %d\n", msg, file, line);
    }
    exit(1);
}
